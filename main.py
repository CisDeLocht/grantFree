...
if __name__ == '__main__':
    ...
    #for i, k in enumerate(users):
         #_, IP_g_m = sim_detection_probability(N, k, M, P, Pilot.GAUSSIAN, Lp, cell_radius, margin, SNR, "MUSIC", s)

        # pool.apply_async(worker, (sim_detection_probability, N, k, M, P, Pilot.GAUSSIAN, Lp, cell_radius, margin, SNR, "MUSIC", s),
        #                  callback=lambda res, idx=i: collect_result(res, P_g_m, IP_g_m, idx))
        # pool.apply_async(worker,
        #                  (sim_detection_probability, N, k, M, P, Pilot.ICBP, Lp, cell_radius, margin,SNR, "MUSIC", s),
        #                  callback=lambda res, idx=i: collect_result(res, P_I_m, IP_I_m, idx))
        # pool.apply_async(worker,
        #                  (sim_detection_probability, N, k, M, P, Pilot.GAUSSIAN, Lp, cell_radius, margin,SNR, "OMP", s),
        #                  callback=lambda res, idx=i: collect_result(res, P_g_o, IP_g_o, idx))
        # pool.apply_async(worker,
        #                  (sim_detection_probability, N, k, M, P, Pilot.ICBP, Lp, cell_radius, margin,SNR, "OMP", s),
    ...
    for i, a in enumerate(antennas):
        pool.apply_async(worker,
                         (sim_detection_probability, N, K, a, P, Pilot.GAUSSIAN, Lp, cell_radius,margin, SNR, "MUSIC", s),
                         callback=lambda res, idx=i: collect_result(res, P_g_m, IP_g_m, idx))
        pool.apply_async(worker,
                         (sim_detection_probability, N, K, a, P, Pilot.ICBP, Lp, cell_radius,margin, SNR, "MUSIC", s),
                         callback=lambda res, idx=i: collect_result(res, P_I_m, IP_I_m, idx))
        pool.apply_async(worker,
                         (sim_detection_probability, N, K, a, P, Pilot.GAUSSIAN, Lp, cell_radius,margin, SNR, "OMP", s),
                         callback=lambda res, idx=i: collect_result(res, P_g_o, IP_g_o, idx))
        pool.apply_async(worker,
                         (sim_detection_probability, N, K, a, P, Pilot.ICBP, Lp, cell_radius,margin, SNR, "OMP", s),
                         callback=lambda res, idx=i: collect_result(res, P_I_o, IP_I_o, idx))
    pool.close()
    pool.join()
    ...
    for i, l in enumerate(pilot_lengths):
        pool.apply_async(worker,
                         (sim_detection_probability, N, K, M, P, Pilot.GAUSSIAN, l, cell_radius,margin, SNR, "MUSIC", s),
                         callback=lambda res, idx=i: collect_result(res, P_g_m, IP_g_m, idx))
        pool.apply_async(worker,
                         (sim_detection_probability, N, K, M, P, Pilot.ICBP, l, cell_radius,margin, SNR, "MUSIC", s),
                         callback=lambda res, idx=i: collect_result(res, P_I_m, IP_I_m, idx))
        pool.apply_async(worker,
                         (sim_detection_probability, N, K, M, P, Pilot.GAUSSIAN, l, cell_radius,margin, SNR, "OMP", s),
                         callback=lambda res, idx=i: collect_result(res, P_g_o, IP_g_o, idx))
        pool.apply_async(worker,
                         (sim_detection_probability, N, K, M, P, Pilot.ICBP, l, cell_radius,margin, SNR, "OMP", s),
                         callback=lambda res, idx=i: collect_result(res, P_I_o, IP_I_o, idx))
    pool.close()
    pool.join()
    ...
    # SNR_list = np.asarray([100, 120, 140, 160, 180, 200, 220, 240, 260, 280, 300])
    ...
    #     pool.apply_async(worker, (sim_detection_probability, N, K, M, P, Pilot.GAUSSIAN, Lp, cell_radius,margin, snr, "MUSIC", s),
    #                      callback=lambda res, idx=i: collect_result(res, P_g_m, IP_g_m, idx))
    #     pool.apply_async(worker, (sim_detection_probability, N, K, M, P, Pilot.ICBP, Lp, cell_radius,margin, snr, "MUSIC", s),
    #                      callback=lambda res, idx=i: collect_result(res, P_I_m, IP_I_m, idx))
    #     pool.apply_async(worker, (sim_detection_probability, N, K, M, P, Pilot.GAUSSIAN, Lp, cell_radius,margin, snr, "OMP", s),
    #                      callback=lambda res, idx=i: collect_result(res, P_g_o, IP_g_o, idx))
    #     pool.apply_async(worker, (sim_detection_probability, N, K, M, P, Pilot.ICBP, Lp, cell_radius,margin, snr, "OMP", s),
    #                      callback=lambda res, idx=i: collect_result(res, P_I_o, IP_I_o, idx))
    # pool.close()
    # pool.join()
    #
    ...
    # plot_detailed_reliability(IP_g_m, IP_g_o, IP_I_m, IP_I_o, "MUSIC", "OMP", SNR_list, "Transmit SNR", "[dB]", x_log=False)
